...
class EmotionDetector @Inject constructor(
    ...
    /**
     * Detects emotion from text using Gemma offline model
     */
        ...
        try {
            ...
            val response = gemmaService.generateTextAsync(
                ...
                UnifiedGemmaService.GenerationConfig(
                    maxTokens = 100,
                    temperature = 0.5f  // Lower temperature for more consistent output
                )
            )
    ...
    suspend fun detectFromMultimodal(
        ...
        when {
            audioEmotion != null && textEmotion != null -> {
                // If both are available, use Gemma to reconcile
    ...
    /**
     * Uses Gemma to reconcile emotions detected from different modalities
     */
    private suspend fun reconcileEmotions(
        ...
        // Use Gemma to make final decision
        val prompt = """
    ...
    suspend fun analyzeEmotionTrajectory(
        ...
        // Use Gemma to analyze the trajectory
